alias gb="git branch"
alias gbd="git branch -D"
alias gc="git commit"
alias gca="git commit -a"
alias gcm="git commit -m"
alias gcam="git commit -a -m"
alias gco="git checkout"
alias gcob="git checkout -b"
alias gcom="git checkout master"
alias gf="git fetch root"
alias gl="git log --pretty=format:'%h %C(yellow)%an%C(reset) %s%C(green)%d%C(reset)'"
alias gld="git log --pretty=format:'%h %C(cyan)%cd %C(yellow)%an%C(reset) %s%C(green)%d%C(reset)' --date=short"
alias gm="git merge"
alias gmm="git merge root/master"
alias gmm!="git checkout master && git merge root/master && git push origin master"
alias gp="gl -5"
alias gpd="gld -5"
alias gr="git remote -v"
alias gs="git status -sb"
alias gundo="git reset HEAD^ --soft"
alias gunstage="git reset HEAD"
alias gshipped="git log --pretty=format:'%h %C(cyan)%cd %C(yellow)%an%C(reset) %s' --date=short --since=1.week | grep \"Merge remote-tracking branch 'amacdougall/\" | sed \"s/Merge remote-tracking branch/merged/\" | sed \"s/'//g\""